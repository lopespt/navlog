...
// Precision used when rounding decimal degrees for storage. 1e6 = 6 casas
// decimais ≈ 11 cm no equador — granularidade muito acima do que GPS de
// aviação geral entrega. Centralizado aqui em vez de espalhar `1e6` em
// cada call site (era usado em airac.js, deviation-panel, waypoint-editor,
// use-flight-actions — agora todos usam roundCoord).
const COORD_ROUND_SCALE = 1e6;

/**
 * Arredonda lat ou lon para a precisão padrão de armazenamento
 * (6 casas decimais). Use sempre que persistir uma coordenada para
 * evitar serializar 17 dígitos de noise IEEE-754.
 * @param {number} v - Latitude ou longitude em graus decimais.
 * @returns {number}
 */
function roundCoord(v) {
  return Math.round(v * COORD_ROUND_SCALE) / COORD_ROUND_SCALE;
}
