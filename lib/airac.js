...
// AIRAC cycle is 28 days; cache 14 = half-cycle so the user is never more
// than one cycle behind a fresh fetch. Tradeoff: more network traffic vs.
// staleness; 14 days is the practical middle ground.
const AIRAC_TTL_MS = 14 * 24 * 60 * 60 * 1000;

// In Node (tests), pull roundCoord from lib/coords.js. In the browser
// it lives on window via the coords UMD footer.
if (typeof roundCoord === "undefined" && typeof require === "function") {
  /* eslint-disable no-global-assign */
  // eslint-disable-next-line no-var
  var { roundCoord } = require("./coords.js");
...
function _airacCacheSet(key, data, ttlMs) {
  try {
    localStorage.setItem(AIRAC_LS_PREFIX + key, JSON.stringify({
      data,
      expiresAt: Date.now() + (ttlMs != null ? ttlMs : AIRAC_TTL_MS),
    }));
...
function _normalizeCoords(d) {
  ...
  if (!d) return d;
  if (d.latitude  != null) d.latitude  = roundCoord(Number(d.latitude) );
  if (d.longitude != null) d.longitude = roundCoord(Number(d.longitude));
  if (d.coordinates) {
    if (d.coordinates.lat != null) d.coordinates.lat = roundCoord(Number(d.coordinates.lat));
    if (d.coordinates.lon != null) d.coordinates.lon = roundCoord(Number(d.coordinates.lon));
  }
  return d;
}
...
async function airacAirport(icao, signal) {
  ...
  try {
    ...
    _airacCacheSet("ap_" + code, d, AIRAC_TTL_MS);
    return d;
  } catch (e) {
    if (e && e.name === "AbortError") throw e;
    return null;
...
async function airacProcedures(icao) {
  ...
  try {
    ...
    const j = await r.json();
    if (j.status !== "success" || !j.data) return null;
    _airacCacheSet("proc_" + code, j.data, AIRAC_TTL_MS);
    return j.data;
  } catch (_) { return null; }
}
...
async function airacProcedureDetail(airport, identifier) {
  ...
  try {
    ...
    const j = await r.json();
    if (j.status !== "success" || !j.data) return null;
    _airacCacheSet(key, j.data, AIRAC_TTL_MS);
    return j.data;
  } catch (_) { return null; }
}
...
function legToCheckpoint(leg, source, typeCode) {
  ...
  const lat = roundCoord(Number(leg.fix_coordinates.lat));
  const lon = roundCoord(Number(leg.fix_coordinates.lon));
  const cp = {
