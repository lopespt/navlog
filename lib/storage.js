...
// Legacy no-op alias. There's still one caller in index.html that hasn't been
// migrated to savePdfOverlayIdb; removing this shim turns that call into a
// ReferenceError, so it stays until the caller is fixed in a separate change.
async function savePdfHandle(id, handle) { /* no-op, use savePdfOverlayIdb */ }
...
const __NAVLOG_STORAGE__ = {
  savePdfOverlayIdb, getPdfOverlayIdb, getAllPdfOverlaysIdb, deletePdfHandle, savePdfHandle,
