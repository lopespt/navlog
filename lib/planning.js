...
// vectorPosition — dead-reckons position along an ATC heading vector.
//
// Wind triangle: given true heading TH and TAS, with wind blowing FROM (WD,Vw),
// the ground vector is (TAS·sin TH − Vw·sin WD, TAS·cos TH − Vw·cos WD), so
// TC = atan2(g_x, g_y) and GS = |g|.
//
// "vector" shape:
//   { heading, headingType: "M"|"T", mode: "open"|"distance"|"time",
//     limitDist?, limitMin?, thenDirectIdx?, startedAt, fromLat, fromLon,
//     currentTas?, currentAlt? }
//
// Returns { lat, lon, course, gs, th, distFlown, timeFlown, distRemaining,
// timeRemaining, limitReached } or null on bad inputs. Position clamps at the
// limit point so the marker doesn't drift past the auto-trigger threshold.
function vectorPosition(args) {
  if (!args) return null;
  var v = args.vector;
  if (!v) return null;
  if (v.fromLat == null || v.fromLon == null || v.heading == null) return null;
  var nowMin = args.nowMin;
  if (nowMin == null) return null;
  var startMin = parseHHMM(v.startedAt);
  if (startMin == null) return null;

  var windDir = args.windDir != null ? args.windDir : 0;
  var windVel = args.windVel != null ? args.windVel : 0;
  var variation = args.variation != null ? args.variation : 0;
  var tas = args.tas != null ? args.tas
          : (v.currentTas != null ? v.currentTas : 100);

  var th = v.headingType === "T" ? v.heading : (v.heading + variation);
  th = ((th % 360) + 360) % 360;

  var thR = toRad(th);
  var wdR = toRad(windDir);
  var ax = tas * Math.sin(thR);
  var ay = tas * Math.cos(thR);
  var wx = -windVel * Math.sin(wdR);
  var wy = -windVel * Math.cos(wdR);
  var gx = ax + wx;
  var gy = ay + wy;
  var tc = ((toDeg(Math.atan2(gx, gy)) % 360) + 360) % 360;
  var gs = Math.sqrt(gx * gx + gy * gy);

  var elapsedMin = nowMin - startMin;
  if (elapsedMin < 0) elapsedMin += 1440;
  if (elapsedMin < 0) elapsedMin = 0;

  var distFlown = (gs * elapsedMin) / 60;
  var timeFlown = elapsedMin;
  var limitReached = false;
  var distRemaining = null;
  var timeRemaining = null;

  if (v.mode === "distance" && v.limitDist != null) {
    if (distFlown >= v.limitDist) {
      limitReached = true;
      distFlown = v.limitDist;
      timeFlown = gs > 0 ? (v.limitDist / gs) * 60 : 0;
    } else {
      distRemaining = v.limitDist - distFlown;
      timeRemaining = gs > 0 ? (distRemaining / gs) * 60 : null;
    }
  } else if (v.mode === "time" && v.limitMin != null) {
    if (elapsedMin >= v.limitMin) {
      limitReached = true;
      timeFlown = v.limitMin;
      distFlown = (gs * v.limitMin) / 60;
    } else {
      timeRemaining = v.limitMin - elapsedMin;
      distRemaining = (gs * timeRemaining) / 60;
    }
  }

  var pos = projectDest(v.fromLat, v.fromLon, tc, distFlown);
  return {
    lat: pos[0], lon: pos[1],
    course: tc, gs: gs, th: th,
    distFlown: distFlown, timeFlown: timeFlown,
    distRemaining: distRemaining, timeRemaining: timeRemaining,
    limitReached: limitReached,
  };
}

// altTransitionState — dead-reckons current altitude during an ATC-cleared
// climb or descent. The aircraft starts at fromAlt at startedAt and changes
// altitude at the aircraft's ROC (climb) or ROD (descent) until reaching
// targetAlt, after which it stays level.
//
// Inputs:
//   activeAlt = { targetAlt, fromAlt, startedAt, fromLat?, fromLon? }
//   nowMin (fractional minutes-of-day)
//   ac (aircraft profile — needs rocClimb / rodDescent)
//
// Returns { phase, currentAlt, totalSeconds, secondsRemaining, reached } or
// null on bad inputs. phase ∈ "climb"|"descent"|"level".
function altTransitionState(args) {
  if (!args) return null;
  var aa = args.activeAlt;
  if (!aa) return null;
  if (aa.targetAlt == null || aa.fromAlt == null) return null;
  var nowMin = args.nowMin;
  if (nowMin == null) return null;
  var startMin = parseHHMM(aa.startedAt);
  if (startMin == null) return null;
  var ac = args.ac || {};

  var delta = Number(aa.targetAlt) - Number(aa.fromAlt);
  if (delta === 0) {
    return {
      phase: "level", currentAlt: Number(aa.targetAlt),
      totalSeconds: 0, secondsRemaining: 0, reached: true,
    };
  }
  var phase = delta > 0 ? "climb" : "descent";
  var rate = phase === "climb"
    ? Number(ac.rocClimb || 500)
    : Number(ac.rodDescent || 500); // ft/min, always positive
  if (!(rate > 0)) rate = 500;

  var totalSeconds = (Math.abs(delta) / rate) * 60;
  var elapsedSec = (nowMin - startMin) * 60;
  if (elapsedSec < -60) elapsedSec += 1440 * 60; // crossed midnight
  if (elapsedSec < 0) elapsedSec = 0;

  var reached = elapsedSec >= totalSeconds;
  var currentAlt;
  var secondsRemaining;
  if (reached) {
    currentAlt = Number(aa.targetAlt);
    secondsRemaining = 0;
  } else {
    var altPerSec = (rate / 60) * (delta > 0 ? 1 : -1);
    currentAlt = Number(aa.fromAlt) + altPerSec * elapsedSec;
    secondsRemaining = totalSeconds - elapsedSec;
  }
  return {
    phase: reached ? "level" : phase,
...
function estimatedPosition(args) {
  ...
  // Active ATC vector overrides everything else: we're not on the planned
  // route, we're flying a heading. The marker rides the vector line.
  var vec = flight.activeVector;
  if (vec && vec.fromLat != null && vec.fromLon != null) {
    var vp = vectorPosition({
      vector: vec, nowMin: nowMin,
      windDir: flight.windDir, windVel: flight.windVel,
      variation: flight.variation, tas: vec.currentTas,
    });
    if (vp) {
      return {
        lat: vp.lat, lon: vp.lon, course: vp.course, frac: 0,
        segment: { from: { lat: vec.fromLat, lon: vec.fromLon }, to: null },
        devActive: false, holding: false, parked: false,
        vectoring: true, vector: vp,
      };
    }
  }
...
const __NAVLOG_PLANNING__ = {
  ...
  estimatedPosition, vectorPosition, altTransitionState,
  applyDirectTo, clearDirectTo, applyHolds,
