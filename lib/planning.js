...
// Classify the phase transition between two adjacent portions:
...
// Returns null when no virtual marker is warranted (e.g. cruise → cruise).
...
// Magnetic declination wrapper around the NOAA WMM library (loaded on the
// browser by index.html as window._geomagnetism). Returns null when the
// library hasn't loaded yet or when called with missing lat/lon — callers
// then fall back to the route-level variation setting. A small in-memory
// cache keyed by tenth-of-a-degree lat/lon + thousands-of-feet altitude
// keeps repeat lookups during a single re-render essentially free; on a
// 10-WP route the computed pipeline previously called this 10× per render.
// Only non-null results are cached so the first call after WMM finishes
// loading still returns a real value.
...
// "HH:MM:SS" or "HH:MM" → "HH:MM" — used by the flight tab and log tab to
// drop the seconds when rendering ATAs / ATDs without losing precision in
// storage.
...
// Current UTC time in the same units as parseHHMM — minutes since midnight,
// with seconds as fractional minute (e.g. 12:34:30 UTC → 754.5). Used by the
// flight tab to compute live deltas and by the map ETA labels.
function nowHHMM() {
  const d = new Date();
...
const __NAVLOG_PLANNING__ = {
  ...
  computeLegPhases, resolveAltitudeProfile, portionTransitionLabel,
