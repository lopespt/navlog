...
// recalibrate panel, plus global zoom-fade prefs. Self-contained React
// component; consumes window globals for shared helpers (renderPdfFromHandle,
// savePdfHandle from lib/pdf.js + lib/storage.js, getPdfHandle is a known
// dead reference from the legacy "Recarregar" button).
//
// Loaded as <script type="text/babel" src="app/pdf-layers-panel.jsx"> BEFORE
// the main babel block in index.html. Ends with an explicit
// `window.PdfLayersPanel = PdfLayersPanel;` so the main script can pick it up
// via a one-line alias.
...
// Expose for the main babel block. Each extracted .jsx ends with an explicit
// window assignment so the consumer can alias it in one line without relying
// on free-variable lookup (which may or may not work depending on whether
// babel-standalone emits strict mode for the consuming script).
window.PdfLayersPanel = PdfLayersPanel;
